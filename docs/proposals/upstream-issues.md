...
— not the mean and standard deviation: the standard deviation does not exist for
`alpha < 2`, the mean only for `alpha > 1`, and the quantiles are robust to the
heavy tails even where the mean is defined —
...
platform ULP drift, and with both fitters on the same float32 tables — five
optima are bit-identical, five improve (the interpolated negative log
likelihood falls by up to 4.48e-08, with parameter moves of at most 3.5e-05),
none get worse. Scored against `calculate_levy` as well, excluding the points
at |z| < 1e-3 where that integrator fails, three of the five moves are lower
and two higher, by at most +2.8e-05 on an NLL of 401 — within the quadrature's
own per-point error of ~1e-7 relative, so it cannot adjudicate moves this
small; the pull request description carries the full table.
...
`tests/test_fit_starting_scale.py`, 60 tests, pins the nine seeds above plus the
never-worse guarantee, pinned-parameter preservation in all five
parametrizations, the start round-tripping to the data's location and scale
in every parametrization, bound compliance from data scales 1e-4 to 1e4 for
the free components with pinned ones left exactly as given, and fallback on
