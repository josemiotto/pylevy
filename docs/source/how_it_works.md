...
at `tan(x_axis)`, so 200 nodes reach out to `x ≈ ±637` (`tan(0.999·π/2)`)
rather than to some arbitrary truncation, and the `0.999` keeps the endpoints
off the pole of the tangent. Beyond the outermost nodes -- and, closer in,
beyond the crossover limits described below -- the table is not used at all:
the power-law tail takes over, and that is what covers the rest of the line.

Working in arctan space is what lets a fixed-size table reach that far, and it
puts the nodes where they are needed: dense near the mode, sparse far out in
the tails where the density is smooth and nearly a power law.
