...
The distribution is ``levy-stable``; the import name is ``levy``. Optional
extras: ``levy-stable[pandas]`` for labeled input and output,
``levy-stable[torch]`` for a differentiable backend. Neither is imported
unless it is installed and used.
