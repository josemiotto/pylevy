...
**Nothing breaks.** Every 1.x name still works, and away from the handful of
bugs listed at the [end of this page](#bugs-that-were-fixed) it returns exactly
the same numbers — bit for bit, verified by installing wheels built from 1.1
and from 2.0 into two environments and comparing 99,312 values. Where a number
does change, it is because 1.x returned a wrong one: NaN from the sampler at
`alpha = 1`, a sample that ignored `mu` and `sigma`, a CDF of `6e+307`.
...
alpha, beta, mu, sigma = 1.5, 0.3, 0.1, 1.2
```
