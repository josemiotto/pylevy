...
#: not only the skewed draws that need it. Module level so that the tests assert
...
def _repair_table(key, table):
    ...
    about 0.0128 apart. A bad cell at either end of the x range has a usable
    neighbour on one side only and is copied from it instead.
    """
    if key != 'cdf':
        return table
...
class Parameters(object):
    ...
    def x(self, values):
        ...
        # check below and then assign rows. Name the actual problem instead.
        # asarray is a no-op on an ndarray, so the values are untouched.
