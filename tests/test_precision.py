...
def _shipped_only(packaged_tables):
    """Every assertion in this module is about the tables that ship."""
...
@pytest.mark.slow
def test_fits_from_the_float32_tables_recover_the_truth():
    """A truth-recovery smoke test, not a float32-versus-float64 comparison.

    That comparison was made once, when the tables were converted: over 40
    seeded samples of n=1000 the recovered parameters moved by at most 0.0044%
    of one sampling standard error. It cannot be repeated here because the
    float64 tables are no longer in the tree. What can be checked on every
    run is the consequence: that fits from the shipped tables still recover
    the truth well within the fitter's own sampling noise, over 12 seeds.
    """
    fits = []
