...
@pytest.mark.parametrize("bad", [np.array(1.5), np.zeros((4, 2))])
def test_parameters_x_setter_rejects_arrays_that_are_not_one_dimensional(bad):
    """A 0-D array raised TypeError from len() ("unsized object"); a 2-D one
    with the right number of rows got as far as the assignment loop and failed
    there, on a row. Both now say what shape was wanted.
    """
    parameters = _free_parameters()
...
def test_repair_leaves_no_nan_in_a_column_with_no_usable_cell():
    """np.clip passes NaN through, so a column of NaN came back as a column of
    NaN -- while the warning said it had been clipped into [0, 1].
    """
    table = np.zeros((4, 2, 2), dtype="float64")
    table[:, 0, 0] = np.nan
    table[:, 1, 1] = -np.inf
    ...
    assert np.all((repaired >= 0.0) & (repaired <= 1.0))
...
def test_edge_cells_are_reported_as_copied_not_interpolated(caplog):
    ...
    assert "interpolating" not in caplog.text


def test_unusable_column_warning_names_grid_indices(caplog):
    """The message used to read `alpha=4 beta=13`, which looks like parameter
    values; they are grid indices, and now say so.
    """
    import logging

    table = np.zeros((4, 3, 5), dtype="float64")
    table[:, 2, 4] = np.nan
    with caplog.at_level(logging.WARNING, logger="levy"):
        levy._repair_table("cdf", table)
    assert "alpha index 2, beta index 4" in caplog.text
